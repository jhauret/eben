...
class TemporalTransforms:
    ...
    def __init__(self, audio: torch.Tensor, sr: int, padding_length: int = 10000,
    ...
    def smoothing(self):
        ...
        smoothing_len = 512
        self._audio[0, :smoothing_len] *= torch.pow(torch.linspace(0, 1, smoothing_len), 2)
        self._audio[0, -smoothing_len:] *= torch.pow(torch.linspace(1, 0, smoothing_len), 2)
    ...
    def remove_hf(self, cutoff_freq: int = 600, q_factor: float = 1):
        ...
        def reverse(x): return torch.flip(input=x, dims=[1])
    ...
    def normalize(self, percent=0.99999):
        ...
        sorted_audio = torch.sort(abs(self._audio))[0].squeeze()  # values of sorted audio
    ...
    def resampling(self, new_freq: int):
        ...
        resampling = Resample(orig_freq=self._sr, resampling_method='kaiser_window',
                              new_freq=new_freq)
    ...
    def select_part(self, len_seconds: float):
        ...
        if real_time_len >= desired_time_len:  # cut tensor
            if not self.determinist:
                start_idx = torch.randint(low=0, high=real_time_len - desired_time_len + 1,
                                          size=(1,))
            else:
                start_idx = int((real_time_len - desired_time_len)/4)
            self._audio = self._audio[:, start_idx:start_idx + desired_time_len]
        else:
            self._audio = torch.cat([self._audio, torch.zeros(1, desired_time_len - real_time_len)],
                                    dim=1)
